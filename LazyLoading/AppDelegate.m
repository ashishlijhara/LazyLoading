...
@interface AppDelegate () {
  ...
  dispatch_queue_t tweetAccessQueue;
  dispatch_source_t fetchTimer;
}
...
- (void)dealloc {
  dispatch_suspend(fetchTimer);
  dispatch_release(fetchTimer);
  dispatch_release(tweetAccessQueue);
  [images release];
  [super dealloc];
}
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
  ...
  tweetAccessQueue = dispatch_queue_create("com.ashish.tweetLoadingQueue", DISPATCH_QUEUE_CONCURRENT);
  ...
  void(^tweetFetchBlock)(void) = ^(void) {
    ...
    dispatch_apply(RESULTS_PAGES, globalBGQueue, ^(size_t index) {
      NSLog(@"Ready to fetch %lu", index + 1);
  ...
  fetchTimer = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0, globalBGQueue);
  dispatch_source_set_timer(fetchTimer, dispatch_time(DISPATCH_TIME_NOW, 0), NSEC_PER_SEC * 15, 0);
  dispatch_source_set_event_handler(fetchTimer, tweetFetchBlock);
  dispatch_resume(fetchTimer);
}

- (void)addTweetItem:(TwitterImageItem *)item {
  dispatch_barrier_async(tweetAccessQueue, ^{
...
- (TwitterImageItem *)tweetItemAtIndex:(NSInteger)index {
  ...
  dispatch_sync(tweetAccessQueue, ^{
    item = [images objectAtIndex:index];
  });
  return item;
}
